...
容器内部使用 HTTP，不再要求绑定域名、申请证书或放置 `cert.pem` / `key.pem`。订阅网关固定绑定宿主机 `127.0.0.1:54444`，管理后台固定绑定宿主机 `127.0.0.1:64444`；公网 HTTPS、域名、反代或隧道由你在 Docker 外部自行处理。

如果机场后端就在同一台宿主机上，并监听了 `0.0.0.0:10086`，机场地址可填写 `http://host.docker.internal:10086`。

如果订阅网关前面还有一层自建 nginx、frp、隧道入口等固定反代机，并且需要日志显示真实访客 IP，可在 `.env` 中配置可信代理 IP：

```env
TRUSTED_PROXY_IPS=203.0.113.10
```

多个 IP 或 CIDR 可用逗号或空格分隔。外层反代需传递 `X-Forwarded-For`，SubSieve 只会信任这里列出的代理来源。
...
| 黑名单 | 增删黑名单 IP（nginx deny 444），增删后立即生效；内置 IDC / 云厂商 CIDR 封禁可开关 |
...
2. **云厂商 IP**：可在 IP 黑名单页开启/关闭；开启后自动识别Ucloud、阿里云、腾讯云、字节、华为云、Google、AWS、Azure、DigitalOcean 等，返回 403
